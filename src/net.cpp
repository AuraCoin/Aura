...
bool AddLocal(const CService& addr, int nScore)
{
    if (!addr.IsRoutable())
		{
		 LogPrint("net", "Address Not Routable \n");
        return false;
		}
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Aura " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Aura is probably already running."), addrBind.ToString());
        else
