...
{
    // Convert Aura:// to Aura:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Aura://"))
    {
        uri.replace(0, 12, "Aura:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Aura.lnk";
}
...
{
    return GetAutostartDir() / "Aura.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Aura-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Aura-qt [" + tr("command-line options") + "]                     " + "\n";
